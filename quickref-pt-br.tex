...
\begin{longtable}{>{\ttfamily}p{0.20\linewidth}p{0.72\linewidth}}
edicao &
...
Na v1.5.4, use o nome canônico do ambiente:
...
O relatório editorial unificado planejado está descrito na issue \#49.
...
Os aliases de compatibilidade incluem \cmd{cwautor}, \cmd{cwassunto}, \cmd{favorcitar} e \cmd{listofassunto}, além das opções em português. Na v1.5.4 o alias do ambiente \texttt{rascunho} está incompleto, pois apenas a macro de início foi associada; use \texttt{cwdraft} até a resolução da issue \#40.

\section*{Limitações conhecidas da v1.5.4}

\begin{itemize}
\item \texttt{tocloft} é hoje um risco importante de compatibilidade; sua remoção está proposta na \#36.
\item \texttt{hyperref} ainda é carregado automaticamente em configurações normais; torná-lo opcional e sanitizar strings de PDF está proposto na \#37.
\item Títulos e outros argumentos móveis precisam de comportamento seguro explícito; veja \#26.
\item Cópias geradas do pacote divergiram historicamente; veja \#43.
\item A matriz formal de compatibilidade com classes padrão, editoriais, UFRJ, COPPE e Poli está proposta na \#55.
\end{itemize}

\section*{Fluxo recomendado}

Use \opt{edicao} durante a escrita, \opt{submeter} para submissão cega quando necessário e \opt{publicar} ou \opt{publicaraceitando} somente depois das decisões editoriais. Mantenha o fonte como registro autoritativo do processo editorial.
